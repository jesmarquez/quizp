...
/**
 * Define all the backup steps that will be used by the backup_quizp_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_quizp_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $quizp = new backup_nested_element('quizp', array('id'), array(
        ...
        $this->add_subplugin_structure('quizpaccess', $quizp, true);
        ...
        $this->add_subplugin_structure('quizpaccess', $attempt, true);

        // Build the tree.
        $quizp->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $quizp->add_child($sections);
        $sections->add_child($section);

        $quizp->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $quizp->add_child($overrides);
        $overrides->add_child($override);

        $quizp->add_child($grades);
        $grades->add_child($grade);

        $quizp->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $quizp->set_source_table('quizp', array('id' => backup::VAR_ACTIVITYID));

        $qinstance->set_source_table('quizp_slots',
                array('quizpid' => backup::VAR_PARENTID));

        $section->set_source_table('quizp_sections',
                array('quizpid' => backup::VAR_PARENTID));

        $feedback->set_source_table('quizp_feedback',
                array('quizpid' => backup::VAR_PARENTID));

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = array('quizp' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('quizp_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('quizp_grades', array('quizp' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {quizp_attempts}
                    WHERE quizp = :quizp AND preview = 0',
                    array('quizp' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $quizp->set_source_alias('attempts', 'attempts_number');
        ...
        $quizp->annotate_files('mod_quizp', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_quizp', 'feedback', 'id');

        // Return the root element (quizp), wrapped into standard activity structure.
        return $this->prepare_activity_structure($quizp);
    }
}
