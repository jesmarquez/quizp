@mod @mod_quizp
Feature: Edit quizpzes where some questions require the previous one to have been completed
  In order to create quizpzes where later questions can only be seen after earlier ones are answered
  As a teacher
  I need to be able to configure this on the Edit quizp page
  ...
  Scenario: The first question cannot depend on the previous (whatever is in the DB)
    Given the following "activities" exist:
      ...
      | quizp       | Quiz 1 | Quiz 1 description | C1     | quizp1    | immediatefeedback  |
  ...
  @javascript
  Scenario: Question dependency cannot apply to deferred feedback quizpzes so UI is hidden
    Given the following "activities" exist:
      | activity   | name   | intro              | course | idnumber | preferredbehaviour |
      | quizp       | Quiz 1 | Quiz 1 description | C1     | quizp1    | deferredfeedback   |
    And the following "questions" exist:
      | questioncategory | qtype       | name                    | questiontext    |
      | Test questions   | truefalse   | TF1                     | First question  |
      | Test questions   | truefalse   | TF2                     | Second question |
  ...
  Scenario: Question dependency cannot apply to questions in a shuffled section so UI is hidden
    Given the following "activities" exist:
      ...
      | quizp       | Quiz 1 | Quiz 1 description | C1     | quizp1    | immediatefeedback  | 2                |
    And the following "questions" exist:
      | questioncategory | qtype       | name | questiontext    |
      | Test questions   | truefalse   | TF1  | First question  |
      | Test questions   | truefalse   | TF2  | Second question |
  ...
  @javascript
  Scenario: Question dependency cannot apply to quizpzes with sequential navigation so UI is hidden
    Given the following "activities" exist:
      | activity   | name   | intro              | course | idnumber | preferredbehaviour | navmethod  |
      | quizp       | Quiz 1 | Quiz 1 description | C1     | quizp1    | immediatefeedback  | sequential |
    And the following "questions" exist:
      | questioncategory | qtype       | name | questiontext    |
      | Test questions   | truefalse   | TF1  | First question  |
      | Test questions   | truefalse   | TF2  | Second question |
  ...
  Scenario: When questions are reordered, the dependency icons are updated correctly
    ...
    And I follow "Quiz 1"
    And I follow "Edit quizp"
    When I move "Question 1" to "After Question 3" in the quizp by clicking the move icon
    Then "Question 2 cannot be attempted until the previous question 1 has been completed • Click to change" "link" should be visible
    And "No restriction on when question 3 can be attempted • Click to change" "link" should be visible
