...
/**
 * This file defines the quizp manual grading report class.
 *
 * @package   quizp_grading
...
require_once($CFG->dirroot . '/mod/quizp/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizp_grading_report extends quizp_default_report {
    ...
    public function display($quizp, $cm, $course) {
        ...
        require_capability('mod/quizp:grade', $this->context);
        $shownames = has_capability('quizp/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('quizp/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this quizp.
        $this->questions = quizp_report_get_significant_questions($quizp);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'quizp_grading');
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'quizp_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $quizp, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "quizpa.quizp = :mangrquizpid AND
                quizpa.preview = 0 AND
                quizpa.state = :statefinished";
        $params = array('mangrquizpid' => $this->cm->instance, 'statefinished' => quizp_attempt::FINISHED);

        $currentgroup = groups_get_activity_group($this->cm, true);
        if ($currentgroup) {
            $users = get_users_by_capability($this->context,
                    array('mod/quizp:reviewmyattempts', 'mod/quizp:attempt'), 'u.id, u.id', '', '', '',
                    $currentgroup, '', false);
            if (empty($users)) {
                $where .= ' AND quizpa.userid = 0';
            } else {
        ...
        return new qubaid_join('{quizp_attempts} quizpa', 'quizpa.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = quizp_attempt::FINISHED;
        $params[] = $this->quizp->id;

        $fields = 'quizpa.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {quizp_attempts} quizpa
                JOIN {user} u ON u.id = quizpa.userid
                WHERE quizpa.uniqueid $asql AND quizpa.state = ? AND quizpa.quizp = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/quizp/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'quizp_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'quizp_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'quizp_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'quizp_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'quizp_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'quizp_grading');
        $table->head[] = get_string('questionname', 'quizp_grading');
        $table->head[] = get_string('tograde', 'quizp_grading');
        $table->head[] = get_string('alreadygraded', 'quizp_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'quizp_grading');
        }
        $table->head[] = get_string('total', 'quizp_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new quizp_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'quizp_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'quizp_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'quizp_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = quizp_get_review_options($this->quizp, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'value' => get_string('saveandnext', 'quizp_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'quizp_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'quizp_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new quizp_attempt($attempt, $this->quizp, $this->cm, $this->course);
            ...
            $events[] = \mod_quizp\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON quizpa.userid = u.id ";
