...
/**
 * This file defines the quizp grades table.
 *
...
require_once($CFG->dirroot . '/mod/quizp/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the quizp grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizp_overview_table extends quizp_attempts_report_table {
     ...
     * @param quizp_overview_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($quizp, $context, $qmsubselect,
            quizp_overview_options $options, $groupstudents, $students, $questions, $reporturl) {
        parent::__construct('mod-quizp-report-overview-report', $quizp , $context,
    ...
    protected function add_average_row($label, $users) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(quizpa.sumgrades) AS grade, COUNT(quizpa.sumgrades) AS numaveraged
                  FROM $from
                 WHERE $where", $params);
        $record->grade = quizp_rescale_grade($record->grade, $this->quizp, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext'=> strip_tags(quizp_report_feedback_for_grade(
                                        $record->grade, $this->quizp->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join($from, 'quizpa.uniqueid', $where, $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = quizp_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->quizp, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = quizp_format_question_grade($this->quizp, $record->grade);
        } else {
            $average = quizp_format_grade($this->quizp, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/quizp:regrade', $this->context)) {
            echo '<input type="submit" name="regrade" value="' .
                    get_string('regradeselected', 'quizp_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = quizp_rescale_grade($attempt->sumgrades, $this->quizp);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = quizp_rescale_grade($newsumgrade, $this->quizp);
            $oldsumgrade = quizp_rescale_grade($oldsumgrade, $this->quizp);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/quizp/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'quizp')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/quizp/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = quizp_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->quizp, 'question');
        }
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'quizp_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'quizp_overview');
        }
    }
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/quizp:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('quizp_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return quizp_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
