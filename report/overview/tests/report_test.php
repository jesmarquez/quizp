...
/**
 * Tests for the quizp overview report.
 *
...
require_once($CFG->dirroot . '/mod/quizp/report/overview/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizp_overview_report_testcase extends advanced_testcase {
    ...
    public function test_report_sql() {
        ...
        $quizpgenerator = $generator->get_plugin_generator('mod_quizp');
        $quizp = $quizpgenerator->create_instance(array('course' => $SITE->id,
                'grademethod' => QUIZP_GRADEHIGHEST, 'grade' => 100.0, 'sumgrades' => 10.0,
                'attempts' => 10));
        ...
        // The test data.
        $fields = array('quizp', 'userid', 'attempt', 'sumgrades', 'state');
        $attempts = array(
            array($quizp->id, $student1->id, 1, 0.0,  quizp_attempt::FINISHED),
            array($quizp->id, $student1->id, 2, 5.0,  quizp_attempt::FINISHED),
            array($quizp->id, $student1->id, 3, 8.0,  quizp_attempt::FINISHED),
            array($quizp->id, $student1->id, 4, null, quizp_attempt::ABANDONED),
            array($quizp->id, $student1->id, 5, null, quizp_attempt::IN_PROGRESS),
            array($quizp->id, $student2->id, 1, null, quizp_attempt::ABANDONED),
            array($quizp->id, $student2->id, 2, null, quizp_attempt::ABANDONED),
            array($quizp->id, $student2->id, 3, 7.0,  quizp_attempt::FINISHED),
            array($quizp->id, $student2->id, 4, null, quizp_attempt::ABANDONED),
            array($quizp->id, $student2->id, 5, null, quizp_attempt::ABANDONED),
        );

        // Load it in to quizp attempts table.
        $uniqueid = 1;
        foreach ($attempts as $attempt) {
            $data = array_combine($fields, $attempt);
            $data['timestart'] = $timestamp + 3600 * $data['attempt'];
            $data['timemodifed'] = $data['timestart'];
            if ($data['state'] == quizp_attempt::FINISHED) {
            ...
            $data['preview'] = 0;
            $DB->insert_record('quizp_attempts', $data);
        }
        ...
        $qmsubselect = quizp_report_qm_filter_select($quizp);
        $reportstudents = array($student1->id, $student2->id, $student3->id);

        // Set the options.
        $reportoptions = new quizp_overview_options('overview', $quizp, $cm, null);
        $reportoptions->attempts = quizp_attempts_report::ENROLLED_ALL;
        $reportoptions->onlygraded = true;
        $reportoptions->states = array(quizp_attempt::IN_PROGRESS, quizp_attempt::OVERDUE, quizp_attempt::FINISHED);

        // Now do a minimal set-up of the table class.
        $table = new quizp_overview_table($quizp, $context, $qmsubselect, $reportoptions,
                array(), $reportstudents, array(1), null);
        $table->define_columns(array('attempt'));
        $table->sortable(true, 'uniqueid');
        $table->define_baseurl(new moodle_url('/mod/quizp/report.php'));
        $table->setup();
