...
/**
 * Base class for the options that control what is visible in an {@link quizp_attempts_report}.
 *
 * @package   mod_quizp
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizp_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the quizp being reported on. */
    public $quizp;

    /** @var object the course module objects for the quizp being reported on. */
    public $cm;

    /** @var object the course settings for the course the quizp is in. */
    public $course;

    /**
     * @var array form field name => corresponding quizp_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => quizp_attempt::IN_PROGRESS,
        'stateoverdue'    => quizp_attempt::OVERDUE,
        'statefinished'   => quizp_attempt::FINISHED,
        'stateabandoned'  => quizp_attempt::ABANDONED,
    );

    /**
     * @var string quizp_attempts_report::ALL_WITH or quizp_attempts_report::ENROLLED_WITH
     *      quizp_attempts_report::ENROLLED_WITHOUT or quizp_attempts_report::ENROLLED_ALL
     */
    public $attempts = quizp_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of quizp_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(quizp_attempt::IN_PROGRESS, quizp_attempt::OVERDUE,
            quizp_attempt::FINISHED, quizp_attempt::ABANDONED);
    ...
    public $pagesize = quizp_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $quizp the settings for the quizp being reported on.
     * @param object $cm the course module objects for the quizp being reported on.
     * @param object $coures the course settings for the coures this quizp is in.
     */
    public function __construct($mode, $quizp, $cm, $course) {
        $this->mode   = $mode;
        $this->quizp   = $quizp;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = quizp_report_should_show_grades($quizp, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/quizp/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('quizp_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('quizp_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == quizp_attempts_report::ALL_WITH) {
                $this->attempts = quizp_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = quizp_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(quizp_attempts_report::ALL_WITH, quizp_attempts_report::ENROLLED_WITH,
                quizp_attempts_report::ENROLLED_WITHOUT, quizp_attempts_report::ENROLLED_ALL))) {
            $this->attempts = quizp_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!quizp_report_can_filter_only_graded($this->quizp)) {
        ...
        if ($this->attempts == quizp_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = quizp_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(quizp_attempt::FINISHED, $this->states);
    }
}
