...
/**
 * Base class for quizp reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class quizp_attempts_report extends quizp_default_report {
    ...
    /** @var object the quizp context. */
    protected $context;

    /** @var mod_quizp_attempts_report_form The settings form to use. */
    protected $form;
     ...
     * @param object $course
     */
    protected function init($mode, $formclass, $quizp, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = quizp_report_qm_filter_select($quizp);

        $this->form = new $formclass($this->get_base_url(),
                array('quizp' => $quizp, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'quizp');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'quizp');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'quizp');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'quizp');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this quizp.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($quizp, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'quizp') . '/' .
                    quizp_format_grade($quizp, $quizp->grade);
        }

        if ($includefeedback && quizp_has_feedback($quizp)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'quizp');
        }
    }
     ...
     * @param mod_quizp_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_quizp_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/quizp:deleteattempts', $this->context);
                    $this->delete_selected_attempts($quizp, $cm, $attemptids, $allowed);
    ...
    /**
     * Delete the quizp attempts
     * @param object $quizp the quizp settings. Attempts that don't belong to
     * this quizp are not deleted.
     ...
     */
    protected function delete_selected_attempts($quizp, $cm, $attemptids, $allowed) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            $attempt = $DB->get_record('quizp_attempts', array('id' => $attemptid));
            if (!$attempt || $attempt->quizp != $quizp->id || $attempt->preview != 0) {
                // Ensure the attempt exists, and belongs to this quizp. If not skip.
                continue;
            }
            ...
            // Set the course module id before calling quizp_delete_attempt().
            $quizp->cmid = $cm->id;
