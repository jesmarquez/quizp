...
/**
 * Base class for the table used by a {@link quizp_attempts_report}.
 *
 * @package   mod_quizp
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class quizp_attempts_report_table extends table_sql {
    ...
    /** @var object the quizp settings for the quizp we are reporting on. */
    protected $quizp;
    ...
    /** @var object mod_quizp_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this quizp.. */
    protected $questions;
     ...
     * @param mod_quizp_attempts_report_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($uniqueid, $quizp, $context, $qmsubselect,
            mod_quizp_attempts_report_options $options, $groupstudents, $students,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/quizp/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'quizp'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return quizp_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = quizp_report_feedback_for_grade(
                quizp_rescale_grade($attempt->sumgrades, $this->quizp, false),
                $this->quizp->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/quizp/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'quizp')));
    ...
    public function base_sql($reportstudents) {
        ...
        $fields = $DB->sql_concat('u.id', "'#'", 'COALESCE(quizpa.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                quizpa.uniqueid AS usageid,
                quizpa.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                quizpa.timefinish,
                quizpa.timestart,
                CASE WHEN quizpa.timefinish = 0 THEN null
                     WHEN quizpa.timefinish > quizpa.timestart THEN quizpa.timefinish - quizpa.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and quizp_attempts tables.
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {quizp_attempts} quizpa ON
                                    quizpa.userid = u.id AND quizpa.quizp = :quizpid";
        $params = array('quizpid' => $this->quizp->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (quizpa.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = quizp_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case quizp_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'quizpa.id IS NOT NULL AND quizpa.preview = 0';
                break;
            case quizp_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND quizpa.preview = 0 AND quizpa.id IS NOT NULL";
                break;
            case quizp_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND quizpa.id IS NULL";
                break;
            case quizp_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND (quizpa.preview = 0 OR quizpa.preview IS NULL)";
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (quizpa.state $statesql OR quizpa.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{quizp_attempts} {$alias}quizpa", "{$alias}quizpa.uniqueid",
                "{$alias}quizpa.quizp = :{$alias}quizpid", array("{$alias}quizpid" => $this->sql->params['quizpid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = quizpa.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'quizpa.uniqueid',
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['quizpa.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a href="javascript:select_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectall', 'quizp') . '</a> / ';
        echo '<a href="javascript:deselect_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectnone', 'quizp') . '</a> ';
        echo '&nbsp;&nbsp;';
        $this->submit_buttons();
        echo '</div>';
