...
/**
 * Post-install script for the quizp statistics report.
 *
 ...
 */
function xmldb_quizp_statistics_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2013092000) {
        ...
        $table = new xmldb_table('quizp_question_statistics');
        ...
        $table = new xmldb_table('quizp_question_response_stats');
        ...
        $table = new xmldb_table('quizp_statistics');
        $field = new xmldb_field('quizpid');
        ...
        upgrade_plugin_savepoint(true, 2013092000, 'quizp', 'statistics');
    }

    if ($oldversion < 2013093000) {
        // Define table quizp_statistics to be dropped.
        $table = new xmldb_table('quizp_statistics');

        // Conditionally launch drop table for quizp_statistics.
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Define table quizp_statistics to be created.
        $table = new xmldb_table('quizp_statistics');

        // Adding fields to table quizp_statistics.
        ...
        // Adding keys to table quizp_statistics.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Conditionally launch create table for quizp_statistics.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Statistics savepoint reached.
        upgrade_plugin_savepoint(true, 2013093000, 'quizp', 'statistics');
    }
