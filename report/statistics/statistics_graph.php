...
/**
 * This script renders the quizp statistics graph.
 *
 * It takes one parameter, the quizp_statistics.id. This is enough to identify the
 * quizp etc.
 *
...
// Check access.
require_login($quizp->course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('quizp/statistics:view', $modcontext);
...
if (empty($currentgroup)) {
    ...
    $groupstudents = get_users_by_capability($modcontext, array('mod/quizp:reviewmyattempts', 'mod/quizp:attempt'),
                                             '', '', '', '', $currentgroup, '', false);
}
$qubaids = quizp_statistics_qubaids_condition($quizpid, $groupstudents, $whichattempts);
...
$graph->parameter['x_label'] = get_string('position', 'quizp_statistics');
...
$fieldstoplot = array(
    ...
    'discriminativeefficiency' => get_string('discriminative_efficiency', 'quizp_statistics')
);
...
foreach (array_keys($fieldstoplot) as $fieldtoplot) {
    ...
    $graph->y_format[$fieldtoplot] = array(
        'colour' => quizp_statistics_graph_get_new_colour(),
        'bar' => 'fill',
        'shadow_offset' => 1,
