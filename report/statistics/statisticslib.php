...
/**
 * Common functions for the quizp statistics report.
 *
...
/**
 * SQL to fetch relevant 'quizp_attempts' records.
 *
 * @param int    $quizpid        quizp id to get attempts for
 ...
 */
function quizp_statistics_attempts_sql($quizpid, $groupstudents, $whichattempts = QUIZP_GRADEAVERAGE, $includeungraded = false) {
    global $DB;

    $fromqa = '{quizp_attempts} quizpa ';

    $whereqa = 'quizpa.quizp = :quizpid AND quizpa.preview = 0 AND quizpa.state = :quizpstatefinished';
    $qaparams = array('quizpid' => (int)$quizpid, 'quizpstatefinished' => quizp_attempt::FINISHED);

    if ($groupstudents) {
        ksort($groupstudents);
        list($grpsql, $grpparams) = $DB->get_in_or_equal(array_keys($groupstudents),
                SQL_PARAMS_NAMED, 'statsuser');
        list($grpsql, $grpparams) = quizp_statistics_renumber_placeholders(
                $grpsql, $grpparams, 'statsuser');
        $whereqa .= " AND quizpa.userid $grpsql";
        $qaparams += $grpparams;
    }

    $whichattemptsql = quizp_report_grade_method_sql($whichattempts);
    if ($whichattemptsql) {
        $whereqa .= ' AND '.$whichattemptsql;
    }

    if (!$includeungraded) {
        $whereqa .= ' AND quizpa.sumgrades IS NOT NULL';
    }
 ...
 */
function quizp_statistics_renumber_placeholders($sql, $params, $paramprefix) {
    $basenumber = null;
    $newparams = array();
...
/**
 * Return a {@link qubaid_condition} from the values returned by {@link quizp_statistics_attempts_sql}.
 *
 * @param int     $quizpid
 ...
 * @return        \qubaid_join
 */
function quizp_statistics_qubaids_condition($quizpid, $groupstudents, $whichattempts = QUIZP_GRADEAVERAGE, $includeungraded = false) {
    list($fromqa, $whereqa, $qaparams) = quizp_statistics_attempts_sql($quizpid, $groupstudents, $whichattempts, $includeungraded);
    return new qubaid_join($fromqa, 'quizpa.uniqueid', $whereqa, $qaparams);
}
 ...
 */
function quizp_statistics_graph_get_new_colour() {
