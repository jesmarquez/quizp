...
/**
 * Standard plugin entry points of the quizp statistics report.
 *
 ...
 * @package  quizp_statistics
 * @category files
 * @param context $previewcontext the quizp context
 ...
 */
function quizp_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('quizp/statistics:view', $context);
 ...
 */
function quizp_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old quizp statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('quizp_statistics', 'timemodified < ?', array($expiretime));
