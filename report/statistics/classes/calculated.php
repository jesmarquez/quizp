...
class calculated {
     ...
     */
    public function get_formatted_quizp_info_data($course, $cm, $quizp) {
        ...
        // General information about the quizp.
        $quizpinfo = array();
        $quizpinfo[get_string('quizpname', 'quizp_statistics')] = format_string($quizp->name);
        $quizpinfo[get_string('coursename', 'quizp_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $quizpinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($quizp->timeopen) {
            $quizpinfo[get_string('quizpopen', 'quizp')] = userdate($quizp->timeopen);
        }
        if ($quizp->timeclose) {
            $quizpinfo[get_string('quizpclose', 'quizp')] = userdate($quizp->timeclose);
        }
        if ($quizp->timeopen && $quizp->timeclose) {
            $quizpinfo[get_string('duration', 'quizp_statistics')] =
                format_time($quizp->timeclose - $quizp->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = quizp_report_scale_summarks_as_percentage($value, $quizp);
                    break;
                case 'number_format_percent':
                    $formattedvalue = quizp_format_grade($quizp, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $quizp->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $quizpinfo[get_string($property, 'quizp_statistics',
    ...
    public function cache($qubaids) {
        ...
        // Store the data.
        $DB->insert_record('quizp_statistics', $toinsert);

    }

    /**
     * Given a record from 'quizp_statistics' table load the data into the properties of this class.
     *
