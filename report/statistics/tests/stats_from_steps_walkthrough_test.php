 ...
 *
 * The quizp stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-quizp-tools/tree/master/statsspreadsheet
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizp_report_statistics_from_steps_testcase extends mod_quizp_attempt_walkthrough_from_csv_testcase {

    /**
     * @var quizp_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    public function test_walkthrough_from_csv($quizpsettings, $csvdata) {
        ...
        $whichattempts = QUIZP_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudents = array();
        list($questions, $quizpstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents);
        if ($quizpsettings['testnumber'] === '00') {
            $this->check_variants_count_for_quizp_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_quizp_stats_for_quizp_00($quizpstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_quizp_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $quizpstats
     */
    protected function check_quizp_stats_for_quizp_00($quizpstats) {
        $quizpstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($quizpstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $quizpstats->$statname, $quizpstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $quizpstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents) {
        $this->report = new quizp_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->quizp);
        list($quizpstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->quizp,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudents,
                                                                                     $questions);

        $qubaids = quizp_statistics_qubaids_condition($this->quizp->id, $groupstudents, $whichattempts);

        // We will create some quizp and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $quizpcalc = new \quizp_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($quizpcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $quizpstats, $questionstats, $qubaids);
        }
        return array($questions, $quizpstats, $questionstats, $qubaids);
    }
