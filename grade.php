...
$cm = get_coursemodule_from_id('quizp', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$quizp = $DB->get_record('quizp', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = quizp_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see quizp reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/quizp/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = quizp_get_user_attempts($quizp->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new quizp_attempt($attempt, $quizp, $cm, $course, false);
...
redirect(new moodle_url('/mod/quizp/report.php', array(
