...
$PAGE->set_url('/mod/quizp/comment.php', array('attempt' => $attemptid, 'slot' => $slot));
...
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'quizp');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/quizp:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'quizp', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'quizp' => format_string($attemptobj->get_quizp_name()), 'user' => fullname($student))));
...
// Quiz name.
$summarydata['quizpname'] = array(
    'title'   => get_string('modulename', 'quizp'),
    'content' => format_string($attemptobj->get_quizp_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'quizp'),
...
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_quizp.init_comment_popup', null, false, quizp_get_js_module());
