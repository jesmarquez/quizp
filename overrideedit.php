...
/**
 * This page handles editing and creation of quizp overrides
 *
 * @package   mod_quizp
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('quizp_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'quizp');
    }
    if (! $quizp = $DB->get_record('quizp', array('id' => $override->quizp))) {
        print_error('invalidcoursemodule');
    }
    list($course, $cm) = get_course_and_cm_from_instance($quizp, 'quizp');
...
$url = new moodle_url('/mod/quizp/overrideedit.php');
if ($action) {
...
// Merge quizp defaults with data.
...
$overridelisturl = new moodle_url('/mod/quizp/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new quizp_override_form($url, $cm, $quizp, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->quizp = $quizp->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $quizp->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('quizp_overrides', $conditions)) {
            ...
            // Set the course module id before calling quizp_delete_override().
            $quizp->cmid = $cm->id;
            quizp_delete_override($quizp, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('quizp_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('quizp_overrides', $fromform);
    ...
    quizp_update_events($quizp, $fromform);
...
// Print the form.
$pagetitle = get_string('editoverride', 'quizp');
