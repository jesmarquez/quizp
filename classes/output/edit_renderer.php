...
/**
 * Renderer outputting the quizp editing UI.
 *
 * @package mod_quizp
...
namespace mod_quizp\output;
defined('MOODLE_INTERNAL') || die();

use \mod_quizp\structure;
use \html_writer;
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \quizp $quizpobj object containing all the quizp settings information.
     * @param structure $structure object containing the structure of the quizp.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\quizp $quizpobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingquizpx', 'quizp',
                format_string($quizpobj->get_quizp_name())), 'editingquizp', 'quizp', '',
                get_string('basicideasofquizp', 'quizp'), 2);

        // Information at the top.
        $output .= $this->quizp_state_warnings($structure);
        $output .= $this->quizp_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->total_marks($quizpobj->get_quizp());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->yui_module('moodle-mod_quizp-quizpquestionbank',
                    'M.mod_quizp.quizpquestionbank.init',
                    array('class' => 'questionbank', 'cmid' => $structure->get_cmid()));

            $popups .= $this->random_question_form($pageurl, $contexts, $pagevars);
            $this->page->requires->yui_module('moodle-mod_quizp-randomquestion',
                    'M.mod_quizp.randomquestion.init');

            $output .= html_writer::div($popups, 'mod_quizp_edit_forms');

            // Include the question chooser.
            $output .= $this->question_chooser();
            $this->page->requires->yui_module('moodle-mod_quizp-questionchooser', 'M.mod_quizp.init_questionchooser');
        }
    ...
    /**
     * Render any warnings that might be required about the state of the quizp,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the quizp structure.
     * @return string HTML to output.
     */
    public function quizp_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function quizp_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'quizp', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'quizpopeningstatus',
    ...
    /**
     * Render the form for setting a quizp' overall grade
     *
     * @param structure $structure the quizp structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     ...
     * @param structure $structure the structure of the quizp being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {

        $header = html_writer::tag('span', get_string('repaginatecommand', 'quizp'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'quizp'),
        );
        if (!$structure->can_be_repaginated()) {
            $buttonoptions['disabled'] = 'disabled';
        } else {
            $this->page->requires->yui_module('moodle-mod_quizp-repaginate', 'M.mod_quizp.repaginate.init');
        }
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'quizp');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'quizp', $select) .
    ...
    /**
     * Render the total marks available for the quizp.
     *
     * @param \stdClass $quizp the quizp settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($quizp) {
        $totalmark = html_writer::span(quizp_format_grade($quizp, $quizp->sumgrades), 'mod_quizp_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'quizp', $totalmark),
     ...
     *
     * @param structure $structure the structure of the quizp being edited.
     * @param \stdClass $section The quizp_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'quizp', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the quizp.
     * @param \stdClass $section data from the quizp_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'quizp');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'quizp') . ' ' . $help,
    ...
    /**
     * Render an icon to remove a section from the quizp.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'quizp', $section->heading);
        $url = new \moodle_url('/mod/quizp/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the quizp.
     *
     * This function calls {@link core_course_renderer::quizp_section_question()}
     *
     * @param structure $structure object containing the structure of the quizp.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_quizp($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-quizp-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'quizp'), array('class' => 'add-menu'));
        ...
        // Disable the link if quizp has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'quizp');
        }
        ...
        // Add a new question to the quizp.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'quizp', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'quizp');
        }
        ...
        $returnurl = new \moodle_url('/mod/quizp/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/quizp/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'quizp', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'quizp');
        }
    ...
    /**
     * Render the form that contains the data for adding a new question to the quizp.
     *
     * @param structure $structure object containing the structure of the quizp.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_quizp(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $quizp the quizp settings from the database.
     * @param \stdClass $question data from the question and quizp_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($quizp, $question, $label = null, $variant = null) {
        $url = quizp_question_preview_url($quizp, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'quizp');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'quizp');
    ...
    /**
     * Render an icon to remove a question from the quizp.
     *
     * @param structure $structure object containing the structure of the quizp.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the quizp.
     *
     * @param structure $structure object containing the structure of the quizp.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('quizpid' => $structure->get_quizpid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'quizp');
            $image = $this->pix_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'quizp');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'quizp', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'quizp'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'quizp', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'quizp'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a quizp edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = quizp_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'quizp').' '.$title));
    ...
    public function random_question(structure $structure, $slot, $pageurl) {
        ...
        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = quizp_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'quizp');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'quizp'), array('class' => 'mod_quizp_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'quizp')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'quizp'),
                )
            )
        );
    ...
    protected function random_question_form(\moodle_url $thispageurl, \question_edit_contexts $contexts, array $pagevars) {
        ...
        $randomform = new \quizp_add_random_form(new \moodle_url('/mod/quizp/addrandom.php'),
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/quizp/edit_rest.php';
        $config->sectionurl = '/mod/quizp/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_quizp-toolboxes',
                'M.mod_quizp.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'quizpid' => $structure->get_quizpid(),
        ...
        $this->page->requires->yui_module('moodle-mod_quizp-toolboxes',
                'M.mod_quizp.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'quizpid' => $structure->get_quizpid(),
        ...
        $this->page->requires->yui_module('moodle-mod_quizp-dragdrop', 'M.mod_quizp.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'quizpid' => $structure->get_quizpid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_quizp-dragdrop', 'M.mod_quizp.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'quizpid' => $structure->get_quizpid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'quizp', '');
     ...
     *
     * @param \mod_quizp\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_quizp\question\bank\custom_view $questionbank, array $pagevars) {
