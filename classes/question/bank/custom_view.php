...
/**
 * Defines the custom question bank view used on the Edit quizp page.
 *
 * @package   mod_quizp
 * @category  question
...
namespace mod_quizp\question\bank;
defined('MOODLE_INTERNAL') || die();


/**
 * Subclass to customise the view of the question bank for the quizp editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the quizp this is used by has been attemptd. */
    protected $quizphasattempts = false;
    /** @var \stdClass the quizp settings. */
    protected $quizp = false;
     ...
     * @param \stdClass $quizp quizp settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $quizp) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->quizpquestionbankcolumns)) {
            $quizpquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $quizpquestionbankcolumns = explode(',', $CFG->quizpquestionbankcolumns);
        }

        foreach ($quizpquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_quizp\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_quizp\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_quizp\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_quizp\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_quizp\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the quizp has been attempted,
     * hence whether some bits of UI, like the add this question to the quizp icon,
     * should be displayed.
     * @param bool $quizphasattempts whether the quizp has attempts.
     */
    public function set_quizp_has_attempts($quizphasattempts) {
        $this->quizphasattempts = $quizphasattempts;
        if ($quizphasattempts && isset($this->visiblecolumns['addtoquizpaction'])) {
            unset($this->visiblecolumns['addtoquizpaction']);
        }
    }

    public function preview_question_url($question) {
        return quizp_question_preview_url($this->quizp, $question);
    }

    public function add_to_quizp_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/quizp/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->quizphasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the quizp.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'value' => get_string('addselectedquestionstoquizp', 'quizp'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/quizp/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'quizp');
