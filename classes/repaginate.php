...
/**
 * Defines the quizp repaginate class.
 *
 * @package   mod_quizp
 ...
 *
 * The quizp setting allows users to write quizpzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the quizp being manipulated. */
    private $quizpid;
    /** @var array the quizp_slots for that quizp. */
    private $slots;

    /**
     * Constructor.
     * @param int $quizpid the id of the quizp being manipulated.
     * @param stdClass[] $slots the quizp_slots for that quizp.
     */
    public function __construct($quizpid = 0, $slots = null) {
        global $DB;
        $this->quizpid = $quizpid;
        if (!$this->quizpid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('quizp_slots', array('quizpid' => $this->quizpid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('quizp_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $quizpslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($quizpslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$quizpslots) {
            return null;
        }
        $newslots = array();
        foreach ($quizpslots as $slot) {
            if ($type == self::LINK) {
