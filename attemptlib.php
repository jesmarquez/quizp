...
/**
 * Back-end code for handling data about quizpzes and the current user's attempt.
 *
 * There are classes for loading all the information about a quizp and attempts,
...
/**
 * Class for quizp exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_quizp_exception extends moodle_exception {
    public function __construct($quizpobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $quizpobj->view_url();
        }
        parent::__construct($errorcode, 'quizp', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a quizp and the questions it contains, and making the
...
class quizp {
    ...
    protected $cm;
    /** @var stdClass the quizp settings from the database. */
    protected $quizp;
    ...
    /** @var array of quizp_section rows. */
    protected $sections = null;
    /** @var quizp_access_manager the access manager for this quizp. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/quizp:preview. */
     ...
     *
     * @param object $quizp the row from the quizp table.
     * @param object $cm the course_module object for this quizp.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($quizp, $cm, $course, $getcontext = true) {
        $this->quizp = $quizp;
        $this->cm = $cm;
        $this->quizp->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new quizp object for a specific user.
     *
     * @param int $quizpid the the quizp id.
     * @param int $userid the the userid.
     * @return quizp the new quizp object
     */
    public static function create($quizpid, $userid = null) {
        global $DB;

        $quizp = quizp_access_manager::load_quizp_and_settings($quizpid);
        $course = $DB->get_record('course', array('id' => $quizp->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('quizp', $quizp->id, $course->id, false, MUST_EXIST);

        // Update quizp with override information.
        if ($userid) {
            $quizp = quizp_update_effective_access($quizp, $userid);
        }

        return new quizp($quizp, $cm, $course);
    }

    /**
     * Create a {@link quizp_attempt} for an attempt at this quizp.
     * @param object $attemptdata row from the quizp_attempts table.
     * @return quizp_attempt the new quizp_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new quizp_attempt($attemptdata, $this->quizp, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this quizp.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                'slot.maxmark, slot.id AS slotid, slot.slot, slot.page',
                '{quizp_slots} slot ON slot.quizpid = :quizpid AND q.id = slot.questionid',
                array('quizpid' => $this->quizp->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this quizp. You must call
    ...
    /**
     * Get an instance of the {@link \mod_quizp\structure} class for this quizp.
     * @return \mod_quizp\structure describes the questions in the quizp.
     */
    public function get_structure() {
        return \mod_quizp\structure::create_for_quizp($this);
    }
    ...
    /** @return int the quizp id. */
    public function get_quizpid() {
        return $this->quizp->id;
    }

    /** @return object the row of the quizp table. */
    public function get_quizp() {
        return $this->quizp;
    }

    /** @return string the name of this quizp. */
    public function get_quizp_name() {
        return $this->quizp->name;
    }

    /** @return int the quizp navigation method. */
    public function get_navigation_method() {
        return $this->quizp->navmethod;
    }

    /** @return int the number of attempts allowed at this quizp (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->quizp->attempts;
    }
    ...
    /** @return object the module context for this quizp. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the quizp,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/quizp:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this quizp.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'quizp', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this quizp.
     * @return array 0, 1, 2, ... => quizp_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('quizp_sections',
                    array('quizpid' => $this->get_quizpid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return quizp_access_manager and instance of the quizp_access_manager class
     * for this quizp at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new quizp_access_manager($this, $timenow,
                    has_capability('mod/quizp:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the quizp context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the quizp context.
     */
    ...
    /**
     * @return string the URL of this quizp's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/quizp/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this quizp's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/quizp/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/quizp/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
        return $url;
    ...
    /**
     * @return string the URL of this quizp's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/quizp/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/quizp/review.php', array('attempt' => $attemptid));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/quizp/summary.php', array('attempt' => $attemptid));
    }
     ...
     * @return string if the quizp policies merit it, return a warning string to
    ...
    public function confirm_start_attempt_message($unfinished) {
        ...
        if ($this->quizp->timelimit && $this->quizp->attempts) {
            return get_string('confirmstartattempttimelimit', 'quizp', $this->quizp->attempts);
        } else if ($this->quizp->timelimit) {
            return get_string('confirmstarttimelimit', 'quizp');
        } else if ($this->quizp->attempts) {
            return get_string('confirmstartattemptlimit', 'quizp', $this->quizp->attempts);
        }
     ...
     *
     * @param int $when One of the mod_quizp_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_quizp_display_options::DURING ||
                $when == mod_quizp_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_quizp_display_options::LATER_WHILE_OPEN && $this->quizp->timeclose &&
                $this->quizp->reviewattempt & mod_quizp_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'quizp',
                    userdate($this->quizp->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'quizp');
        }
    }

    /**
     * @param string $title the name of this particular quizp page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_quizp_exception($this, 'questionnotloaded', $id);
        }
    }
}
...
/**
 * This class extends the quizp class to hold data about the state of a particular attempt,
 * in addition to the data about the quizp.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
...
class quizp_attempt {
    ...
    /** @var int maximum number of slots in the quizp for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var quizp object containing the quizp settings. */
    protected $quizpobj;

    /** @var stdClass the quizp_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this quizp attempt. */
    protected $quba;
    ...
    /** @var array of quizp_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_quizp_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the quizp_attempts table.
     * @param object $quizp the quizp object for this attempt and user.
     * @param object $cm the course_module object for this quizp.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $quizp, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->quizpobj = new quizp($quizp, $cm, $course);
        ...
        $this->slots = $DB->get_records('quizp_slots',
                array('quizpid' => $this->get_quizpid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('quizp_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('quizp_attempts', $conditions, '*', MUST_EXIST);
        $quizp = quizp_access_manager::load_quizp_and_settings($attempt->quizp);
        $course = $DB->get_record('course', array('id' => $quizp->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('quizp', $quizp->id, $course->id, false, MUST_EXIST);

        // Update quizp with override information.
        $quizp = quizp_update_effective_access($quizp, $attempt->userid);

        return new quizp_attempt($attempt, $quizp, $cm, $course);
    }

    /**
     * Static function to create a new quizp_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new quizp_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return quizp_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'quizp');
                }
    ...
    public function get_quizpobj() {
        return $this->quizpobj;
    }
    ...
    /** @return int the course id. */
    public function get_course() {
        return $this->quizpobj->get_course();
    }
    ...
    /** @return string the name of this quizp. */
    public function get_quizp_name() {
        return $this->quizpobj->get_quizp_name();
    }

    /** @return int the quizp navigation method. */
    public function get_navigation_method() {
        return $this->quizpobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->quizpobj->get_cm();
    }
     ...
     */
    public function is_preview_user() {
        return $this->quizpobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this quizp (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->quizpobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this quizp. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->quizpobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the quizp_attempts table. */
    ...
    /** @return string one of the quizp_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/quizp:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the quizp in a non-trivial way?
     ...
     * To see how this works, try setting a quizp to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the quizp
     ...
     *
     * @param question_display_options $options the display options for this quizp attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return quizp_feedback_for_grade($grade, $this->get_quizp(),
                $this->quizpobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->quizpobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->quizpobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_quizp_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/quizp:attempt';
        } else {
            $capability = 'mod/quizp:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a quizp
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the quizp as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/quizp:viewreports') ||
                $this->has_capability('mod/quizp:preview')) {
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case QUIZP_NAVMETHOD_FREE:
                return true;
                break;
            case QUIZP_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return quizp_attempt_state($this->get_quizp(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_quizp_display_options for this quizp at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = quizp_get_review_options($this->get_quizp(),
                        $this->attempt, $this->quizpobj->get_context());
            ...
            $options = mod_quizp_display_options::make_from_quizp($this->get_quizp(),
                    mod_quizp_display_options::DURING);
            $options->flags = quizp_get_flag_option($this->attempt, $this->quizpobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the quizp.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the quizp, or for the
     * whole quizp.
     *
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != QUIZP_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_quizp()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the quizp where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the quizp this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the quizp.
     */
    public function get_question_mark($slot) {
        return quizp_format_question_grade($this->get_quizp(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->quizpobj->get_quizp()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->quizpobj->get_quizp()->timelimit;
        }
        if ($this->quizpobj->get_quizp()->timeclose) {
            $deadlines[] = $this->quizpobj->get_quizp()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->quizpobj->get_quizp()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string quizp view url.
     */
    public function view_url() {
        return $this->quizpobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->quizpobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this quizp's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/quizp/summary.php', array('attempt' => $this->attempt->id));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/quizp/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->quizpobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'quizp'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_quizp_renderer $renderer the quizp renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_quizp_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_quizp-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_quizp_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'quizp', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'quizp');
        ...
        $placeholderqa->start($this->get_quizp()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this quizp attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_quizp_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, quizp_attempt_nav_panel or quizp_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole quizp on one page. (Used by review.php)
     * @return quizp_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_quizp_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_quizp_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_quizp_navblock_title';
        $bc->title = html_writer::span(get_string('quizpnavigation', 'quizp'), '', array('id' => 'mod_quizp_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this quizp.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_quizp_links_to_other_attempts} object returned contains an
     ...
     * @return mod_quizp_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = quizp_get_user_attempts($this->get_quizp()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_quizp_links_to_other_attempts();
     ...
     * @return mod_quizp_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->quizpobj->get_quizp()->graceperiod;
        ...
        // Otherwise, we were in quizp_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->quizpobj->get_quizp()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('quizp_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            quizp_save_best_grade($this->get_quizp(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_quizp\question\qubaids_for_users_attempts(
                $this->get_quizpid(), $this->get_userid());
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            quizp_save_best_grade($this->get_quizp(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_quizp\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('quizp_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_quizp\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        quizp_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_quizp\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $quizprecord = $this->get_quizp();
        $params = array(
            'context' => $this->get_quizpobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'quizpid' => $quizprecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('quizp', $this->get_quizp());
        $event->add_record_snapshot('quizp_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the quizp.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/quizp/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/quizp/' . $script . '.php' . $fragment,
 ...
 * @since      Moodle 2.9
 */
class quizp_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class quizp_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class quizp_nav_panel_base {
    /** @var quizp_attempt */
    ...
    public function __construct(quizp_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the quizp navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new quizp_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new quizp_nav_question_button();
            $button->id          = 'quizpnavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'quizp');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'quizp');
        } else {
            return get_string('viewed', 'quizp');
        }
    }

    public function render_before_button_bits(mod_quizp_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_quizp_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_quizp()->showuserpicture == QUIZP_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_quizp()->showuserpicture == QUIZP_SHOWIMAGE_LARGE) {
...
/**
 * Specialisation of {@link quizp_nav_panel_base} for the attempt quizp page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class quizp_attempt_nav_panel extends quizp_nav_panel_base {
    ...
    public function render_before_button_bits(mod_quizp_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'quizp'),
                array('id' => 'quizpnojswarning'));
    }

    public function render_end_bits(mod_quizp_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'quizp'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link quizp_nav_panel_base} for the review quizp page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class quizp_review_nav_panel extends quizp_nav_panel_base {
    public function get_question_url($slot) {
