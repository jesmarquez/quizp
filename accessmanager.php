...
/**
 * Classes to enforce the various access rules that can apply to a quizp.
 *
 * @package   mod_quizp
 ...
 * quizp, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class quizp_access_manager {
    /** @var quizp the quizp settings object. */
    protected $quizpobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of quizp_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular quizp.
     * @param object $quizpobj An instance of the class quizp from attemptlib.php.
     *      The quizp we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/quizp:ignoretimelimits', ...)).
     */
    public function __construct($quizpobj, $timenow, $canignoretimelimits) {
        $this->quizpobj = $quizpobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($quizpobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular quizp.
     * @param quizp $quizpobj information about the quizp in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/quizp:ignoretimelimits capability.
     * @return array of {@link quizp_access_rule_base}s.
     */
    protected function make_rules($quizpobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($quizpobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['quizpaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('quizpaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the quizp table.
     *
     * @param mod_quizp_mod_form $quizpform the quizp settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_quizp_mod_form $quizpform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($quizpform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'quizp'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_quizp_mod_form $quizpform the quizp form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_quizp_mod_form $quizpform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $quizpform);
        }
    ...
    /**
     * Save any submitted settings when the quizp settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the quizp table.
     *
     * @param object $quizp the data from the quizp form, including $quizp->id
     *      which is the id of the quizp being saved.
     */
    public static function save_settings($quizp) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($quizp);
        }
    }

    /**
     * Delete any rule-specific settings when the quizp is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the quizp table.
     *
     * @param object $quizp the data from the database, including $quizp->id
     *      which is the id of the quizp being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($quizp) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($quizp);
        }
    }
     ...
     */
    protected static function get_load_sql($quizpid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{quizp} quizp';
        $allparams = array('quizpid' => $quizpid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($quizpid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE quizp.id = :quizpid", $allparams);
    }
     ...
     */
    public static function load_settings($quizpid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($quizpid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($quizpid);
        }
    ...
    /**
     * Load the quizp settings and any settings required by the access rules.
     ...
     *
     * @param int $quizpid the quizp id.
     * @return object mdl_quizp row with extra fields.
     */
    public static function load_quizp_and_settings($quizpid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($quizpid, $rules, 'quizp.*');
        $quizp = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($quizpid) as $name => $value) {
                $quizp->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this quizp, such
     * as is shown at the top of the quizp view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this quizp now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_quizp_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        return new mod_quizp_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'quizpobj' => $this->quizpobj,
     ...
     * quizp. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant quizp_attempts row.
    ...
    /**
     * @return bolean if this quizp should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the quizp view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_quizp_renderer $output the quizp renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->quizpobj->view_url(), $message);
            die();
        } else {
            redirect($this->quizpobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the quizp, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(quizp_attempt::IN_PROGRESS, quizp_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = quizp_attempt_state($this->quizpobj->get_quizp(), $attempt);
        $reviewoptions = mod_quizp_display_options::make_from_quizp(
                $this->quizpobj->get_quizp(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->quizpobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->quizpobj->review_url($attempt->id),
