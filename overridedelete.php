...
/**
 * This page handles deleting quizp overrides
 *
 * @package    mod_quizp
...
if (! $cm = get_coursemodule_from_instance("quizp", $quizp->id, $quizp->course)) {
    print_error('invalidcoursemodule');
}
...
$url = new moodle_url('/mod/quizp/overridedelete.php', array('id'=>$override->id));
$confirmurl = new moodle_url($url, array('id'=>$override->id, 'confirm'=>1));
$cancelurl = new moodle_url('/mod/quizp/overrides.php', array('cmid'=>$cm->id));
...
$stroverride = get_string('override', 'quizp');
...
if ($override->groupid) {
    ...
    $confirmstr = get_string("overridedeletegroupsure", "quizp", $group->name);
} else {
    $namefields = get_all_user_name_fields(true);
    $user = $DB->get_record('user', array('id' => $override->userid),
            'id, ' . $namefields);
    $confirmstr = get_string("overridedeleteusersure", "quizp", fullname($user));
}
