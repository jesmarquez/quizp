YUI.add('moodle-mod_quizp-toolboxes', function (Y, NAME) {
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'quizp', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a quizp in editing mode.
 *
 * @module mod_quizp-resource-toolbox
 * @namespace M.mod_quizp.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_quizp.quizpbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'quizp', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_quizp.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_quizp.util.slot.remove(element);
    ...
    edit_maxmark : function(ev, button, activity) {
        ...
        var instancemaxmark  = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class'   : 'resource',
                'field'   : 'getmaxmark',
                'id'      : Y.Moodle.mod_quizp.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_quizp.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_quizp.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_quizp.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_quizp.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_quizp.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_quizp.util.slot.reorderSlots();
        Y.Moodle.mod_quizp.util.slot.reorderPageBreaks();
        Y.Moodle.mod_quizp.util.page.reorderPages();
        Y.Moodle.mod_quizp.util.slot.updateOneSlotSections();
        Y.Moodle.mod_quizp.util.slot.updateAllDependencyIcons();
    },

    NAME : 'mod_quizp-resource-toolbox',
    ATTRS : {
        courseid : {
            'value' : 0
        },
...
M.mod_quizp.resource_toolbox = null;
M.mod_quizp.init_resource_toolbox = function(config) {
    M.mod_quizp.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_quizp.resource_toolbox;
};
/**
 ...
 *
 * @module moodle-mod_quizp-toolboxes
 * @namespace M.mod_quizp.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a quizp.
 *
 * @class section
 * @constructor
 * @extends M.mod_quizp.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'quizp', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'quizp', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'quizp', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'quizp', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'quizp', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_quizp-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_quizp.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_quizp-quizpbase",
        "moodle-mod_quizp-util-slot",
