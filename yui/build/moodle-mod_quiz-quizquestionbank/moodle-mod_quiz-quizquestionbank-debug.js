YUI.add('moodle-mod_quizp-quizpquestionbank', function (Y, NAME) {
...
/**
 * Add questions from question bank functionality for a popup in quizp editing page.
 *
 * @package   mod_quizp
...
var CSS = {
        ...
        ADDTOQUIZPCONTAINER: 'td.addtoquizpaction',
...
Y.extend(POPUP, Y.Base, {
    ...
    create_dialogue: function() {
        ...
        var config = {
            ...
            postmethod: 'form',
            footerContent: null,
            extraClasses: ['mod_quizp_qbank_dialogue']
        };
    ...
    load_content : function(queryString) {
        Y.log('Starting load.', 'debug', 'moodle-mod_quizp-quizpquestionbank');
        this.dialogue.bodyNode.append(this.loadingDiv);

        // If to support old IE.
        if (window.history.replaceState) {
            window.history.replaceState(null, '', M.cfg.wwwroot + '/mod/quizp/edit.php' + queryString);
        }

        Y.io(M.cfg.wwwroot + '/mod/quizp/questionbank.ajax.php' + queryString, {
            method: 'GET',
            on: {
        ...
        Y.log('Load request sent.', 'debug', 'moodle-mod_quizp-quizpquestionbank');
    },
    ...
    load_done: function(transactionid, response) {
        ...
        Y.log('Load completed.', 'debug', 'moodle-mod_quizp-quizpquestionbank');
    ...
    load_failed: function() {
        Y.log('Load failed.', 'debug', 'moodle-mod_quizp-quizpquestionbank');
    },

    link_clicked: function(e) {
        // Add question to quizp. mofify the URL, then let it work as normal.
        if (e.currentTarget.ancestor(CSS.ADDTOQUIZPCONTAINER)) {
...
M.mod_quizp.quizpquestionbank = M.mod_quizp.quizpquestionbank || {};
M.mod_quizp.quizpquestionbank.init = function() {
    return new POPUP();
};
