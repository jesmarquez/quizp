YUI.add('moodle-mod_quizp-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/quizp/mod_form.php.
 *
 * @module moodle-mod_quizp-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function () {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing quizp.
            return;
        }
...
// Ensure that M.mod_quizp exists and that coursebase is initialised correctly
M.mod_quizp = M.mod_quizp || {};
M.mod_quizp.modform = M.mod_quizp.modform || new MODFORM();
M.mod_quizp.modform.init = function() {
    return new MODFORM();
};
