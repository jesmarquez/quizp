YUI.add('moodle-mod_quizp-util-page', function (Y, NAME) {

/**
 * A collection of utility classes for use with pages.
 *
 * @module moodle-mod_quizp-util
 * @submodule moodle-mod_quizp-util-page
 */

Y.namespace('Moodle.mod_quizp.util.page');

/**
 * A collection of utility classes for use with pages.
 *
 * @class Moodle.mod_quizp.util.page
 * @static
 */
Y.Moodle.mod_quizp.util.page = {
    CSS: {
        PAGE : 'page'
    },
    CONSTANTS: {
     ...
     */
    getPages: function() {
        return Y.all(Y.Moodle.mod_quizp.util.slot.SELECTORS.PAGECONTENT + ' ' +
                     Y.Moodle.mod_quizp.util.slot.SELECTORS.SECTIONUL + ' ' +
                    this.SELECTORS.PAGE);
    },
    ...
    add: function(beforenode) {
        ...
        var pagehtml = M.mod_quizp.resource_toolbox.get('config').pagehtml;
        ...
        YUI().use('dd-drop', function(Y) {
            var drop = new Y.DD.Drop({
                node: page,
                groups: M.mod_quizp.dragres.groups
            });
            page.drop = drop;
        });
    ...
    remove: function(page, keeppagebreak) {
        ...
        var previousslot = page.previous(Y.Moodle.mod_quizp.util.slot.SELECTORS.SLOT);
        if (!keeppagebreak && previousslot) {
            Y.Moodle.mod_quizp.util.slot.removePageBreak(previousslot);
        }
        page.remove();
    },
