...
/**
 * Fallback page of /mod/quizp/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $quizp, $pagevars) =
        question_edit_setup('editq', '/mod/quizp/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/quizp/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new quizp_add_random_form(new moodle_url('/mod/quizp/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    quizp_add_random_questions($quizp, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories);
...
// Setup $PAGE.
$streditingquizp = get_string('editinga', 'moodle', get_string('modulename', 'quizp'));
$PAGE->navbar->add($streditingquizp);
$PAGE->set_title($streditingquizp);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$quizpname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoquizp', 'quizp', $quizpname), 2);
$mform->display();
echo $OUTPUT->footer();
