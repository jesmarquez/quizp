...
/**
 * Base class for rules that restrict the ability to attempt a quizp.
 *
 * @package   mod_quizp
...
/**
 * A base class that defines the interface for the various quizp access rules.
 ...
 * @since     Moodle 2.2
 */
abstract class quizp_access_rule_base {
    /** @var stdClass the quizp settings. */
    protected $quizp;
    /** @var quizp the quizp object. */
    protected $quizpobj;
    /** @var int the time to use as 'now'. */
    protected $timenow;

    /**
     * Create an instance of this rule for a particular quizp.
     * @param quizp $quizpobj information about the quizp in question.
     * @param int $timenow the time that should be considered as 'now'.
     */
    public function __construct($quizpobj, $timenow) {
        $this->quizpobj = $quizpobj;
        $this->quizp = $quizpobj->get_quizp();
        $this->timenow = $timenow;
    }

    /**
     * Return an appropriately configured instance of this rule, if it is applicable
     * to the given quizp, otherwise return null.
     * @param quizp $quizpobj information about the quizp in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/quizp:ignoretimelimits capability.
     * @return quizp_access_rule_base|null the rule, if applicable, else null.
     */
     ...
     *
     * @param mod_quizp_preflight_check_form $quizpform the form being built.
    ...
    /**
     * This is called when the current attempt at the quizp is finished. This is
    ...
    /**
     * Information, such as might be shown on the quizp view page, relating to this restriction.
     ...
     * this quizp, then return true. This is used so we can know whether to display a
    ...
    /**
     * Add any fields that this rule requires to the quizp settings form. This
     * method is called from {@link mod_quizp_mod_form::definition()}, while the
    ...
    /**
     * @return array key => lang string any choices to add to the quizp Browser
    ...
    /**
     * Save any submitted settings when the quizp settings form is submitted. This
     * is called from {@link quizp_after_add_or_update()} in lib.php.
    ...
    /**
     * Delete any rule-specific settings when the quizp is deleted. This is called
     * from {@link quizp_delete_instance()} in lib.php.
     ...
     * here is probalby to read the code of {@link quizp_access_manager::load_settings()}.
     *
     * If you have some settings that cannot be loaded in this way, then you can
     * use the {@link get_extra_settings()} method instead, but that has
     * performance implications.
     *
     * @param int $quizpid the id of the quizp we are loading settings for. This
     *     can also be accessed as quizp.id in the SQL. (quizp is a table alisas for {quizp}.)
     ...
     */
    public static function get_settings_sql($quizpid) {
        return array('', '', array());
    }
     ...
     */
    public static function get_extra_settings($quizpid) {
        return array();
    }
}
