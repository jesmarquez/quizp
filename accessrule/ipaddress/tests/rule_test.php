...
/**
 * Unit tests for the quizpaccess_ipaddress plugin.
 *
 * @package    quizpaccess
...
global $CFG;
require_once($CFG->dirroot . '/mod/quizp/accessrule/ipaddress/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizpaccess_ipaddress_testcase extends basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $quizp->subnet = getremoteaddr(null);
        if (!empty($quizp->subnet)) {
            $quizpobj = new quizp($quizp, $cm, null);
            $rule = new quizpaccess_ipaddress($quizpobj, 0);
