...
/**
 * Unit tests for the quizpaccess_timelimit plugin.
 *
 * @package    quizpaccess
...
global $CFG;
require_once($CFG->dirroot . '/mod/quizp/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizpaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $quizp = new stdClass();
        $quizp->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $quizpobj = new quizp($quizp, $cm, null);
        $rule = new quizpaccess_timelimit($quizpobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('quizptimelimit', 'quizpaccess_timelimit', format_time(3600)));
